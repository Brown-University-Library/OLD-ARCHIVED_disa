...
def edit_record(recId=None):
    locs = models.RecordLocation.query.all()
    ...
    allowed_natns = { 'Britain', 'Spain', 'France', 'Honduras', 'United States' }
    uniq_natl_ctx = { (l.location.name, l.location_id)
        for l in locs if l.location_rank == 0 and l.location.name in allowed_natns }
    uniq_cols = { (l.location.name, l.location_id)
        for l in locs if l.location_rank == 1 }
    uniq_town = { (l.location.name, l.location_id)
        for l in locs if l.location_rank == 2 }
    uniq_addl = { (l.location.name, l.location_id)
        for l in locs if l.location_rank == 3 and l.location_id is not None}
    natl_ctx = [ {'id': loc[1], 'value': loc[0],'label': loc[0] }
        for loc in uniq_natl_ctx ]
    ...
    if not recId:
        ...
        return render_template(
            ...
            natl_ctx=natl_ctx, col_state=col_state,
